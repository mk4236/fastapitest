<script>
    import fastapi from "../lib/api";
    import { page, is_login, search_str } from "../lib/store";
    import { link } from "svelte-spa-router";
    import moment from "moment/min/moment-with-locales";

    moment.locale("ko");

    let questions = [];
    let size = 10;
    let total = 0;

    $: total_page = Math.ceil(total / size);

    function get_questions(_page) {
        let params = {
            page: _page,
            size: size,
            search_str: $search_str,
        };
        fastapi("get", "/api/question", params, (json) => {
            questions = json.questions;
            $page = _page;
            total = json.total;
        });
    }

    // 실시간 처리
    // $: $page, $search_str, get_questions($page);
    get_questions($page);
</script>

<div class="container my-3">
    <div class="row my-3">
        <div class="col-6">
            <a
                use:link
                href="/question-create"
                class="btn btn-primary {$is_login ? '' : 'disabled'}"
                >질문 등록하기</a
            >
        </div>
        <div class="col-6">
            <div class="input-group">
                <input
                    type="text"
                    class="form-control"
                    bind:value={$search_str}
                    on:keydown={(event) => {
                        if (event.key === "Enter") {
                            get_questions(0);
                        }
                    }}
                />
                <button
                    class="btn btn-outline-secondary"
                    on:click={() => get_questions(0)}
                >
                    찾기
                </button>
            </div>
        </div>
    </div>
    <table class="table">
        <thead>
            <tr class="text-center table-dark">
                <th>번호</th>
                <th style="width:50%">제목</th>
                <th>작성자</th>
                <th>작성일시</th>
            </tr>
        </thead>
        <tbody>
            {#each questions as question, i}
                <tr class="text-center">
                    <td>{total - $page * size - i}</td>
                    <td class="text-start">
                        <a use:link href="/detail/{question.id}">
                            {question.title}
                        </a>
                        {#if question.answers.length > 0}
                            <span class="small mx-2">
                                [{question.answers.length}]
                            </span>
                        {/if}
                    </td>
                    <td>{question.user ? question.user.username : "-"}</td>
                    <td
                        >{moment(question.created_at).format(
                            "YYYY-MM-DD hh:mm a"
                        )}</td
                    >
                </tr>
            {/each}
        </tbody>
    </table>
    <!-- 페이징처리 시작 -->
    <ul class="pagination justify-content-center">
        <!-- 이전페이지 -->
        <li class="page-item {$page <= 0 && 'disabled'}">
            <button class="page-link" on:click={() => get_questions(page - 1)}
                >이전</button
            >
        </li>
        <!-- 페이지번호 -->
        {#each Array(total_page) as _, loop_page}
            {#if loop_page >= $page - 2 && loop_page <= $page + 2}
                <li class="page-item {loop_page === $page && 'active'}">
                    <button
                        on:click={() => get_questions(loop_page)}
                        class="page-link">{loop_page + 1}</button
                    >
                </li>
            {/if}
        {/each}
        <!-- 다음페이지 -->
        <li class="page-item {$page >= total_page - 1 && 'disabled'}">
            <button class="page-link" on:click={() => get_questions($page + 1)}
                >다음</button
            >
        </li>
    </ul>
    <!-- 페이징처리 끝 -->
    <a
        use:link
        href="/question-create"
        class="btn btn-primary {$is_login ? '' : 'disabled'}">질문 등록하기</a
    >
</div>
